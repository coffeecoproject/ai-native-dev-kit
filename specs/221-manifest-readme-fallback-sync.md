...
1. `intentos-manifest.json` must set `intentOSVersion` and `compatibilityPolicy.phase` to the same current version.
2. `scripts/check-manifest.mjs` must fail when `compatibilityPolicy.phase` differs from `intentOSVersion`.
3. `scripts/check-intentos.mjs` must assert the same manifest phase rule.
