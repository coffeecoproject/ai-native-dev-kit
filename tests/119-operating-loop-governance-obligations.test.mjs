...
test("[verify:data-model-data-model-check-data-model-historical-records-migrat] tests/119-operating-loop-governance-obligations.test.mjs :: exact candidate adds no product persistence model or migration path", () => {
  const changedPaths = execFileSync("git", ["diff", "--name-only", "3a5165733bcc942a706153a350e1678c9924cf06", "--", "."], {
    cwd: root,
    encoding: "utf8",
  }).trim().split(/\r?\n/).filter(Boolean);
  const productDataSegments = new Set([
    "db", "database", "migration", "migrations", "model", "models", "persistence", "storage", "seed", "seeds",
  ]);
  const productDataSuffixes = [".sql", ".prisma"];
  const productDataPaths = changedPaths.filter((relative) => {
    const lower = relative.toLowerCase();
    const named = lower.split("/").some((segment) => productDataSegments.has(segment));
    const typed = productDataSuffixes.some((suffix) => lower.endsWith(suffix));
    return (named || typed)
      && !lower.startsWith("evidence/")
      && !lower.startsWith("schemas/artifacts/");
  });
  assert.deepEqual(productDataPaths, []);
});
