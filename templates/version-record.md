...
- `1.89.1` hardens the adoption apply chain: Adoption Assurance now verifies
  ...
  target paths, human approval, readiness, project, and task context.
- `1.89.1` also requires `init-project --apply-plan` to provide an explicit
  `--approval-record`, with action IDs and target paths matched before any
  write is applied.
- `1.89.1` remains non-authorizing. It blocks stale, unapproved, or mismatched
  apply evidence from being treated as verified adoption; it does not approve
  implementation, native apply, commit, push, release, production, tests,
...
- `1.85.1` hardens Task Governance Consumer Integration: strict consumers now
  ...
  task refs for Closure/User Delivery views, and require structured resume
  review evidence before non-current continuation.
...
- `1.84.0` adds Existing Project Work Queue Takeover: old project TODOs,
  ...
  IntentOS Work Queue recommendation.
- `1.84.0` remains non-authorizing. It does not write target-project files,
  delete old task sources, approve implementation, approve completion, approve
  commit/push, approve release/production, claim full adoption, or install
  native assets.
...
- `1.82.0` adds Controlled Native Adoption Review. `adopt-review` recommends
  ...
  adoption.
- `1.81.3` keeps old-project adoption Human Summary output in plain language
  while preserving raw enums in JSON, Outcome, and Technical Trace.
- `1.81.3` documents `adopt` / `adopt-check` in the script reference and
...
- `1.80.1` remains non-authorizing. It does not approve release, deploy, submit app-store or mini-program review, execute migrations, record secrets, change DNS/payment/CI, or prove real-user stability.
...
- `1.77.2` remains non-executing: it does not run tests, approve implementation, approve release/production, or make Execution Assurance require Test Evidence by default.
- `1.77.1` tightens Test Evidence Binding: command/test-report evidence records `exit_code` and `failure_reason`; passed evidence must resolve to an artifact with matching digest; required Verification Plan test-correctness controls must be preserved.
- `1.77.1` keeps Test Evidence non-executing: it records and checks evidence binding only; it does not run tests, approve implementation, approve release/production, or prove product correctness by itself.
- `1.77.0` adds Test Evidence Binding: Verification Plans can be linked to concrete, task-bound Test Evidence Reports, with strict checks for required obligation coverage, current evidence, source bindings, command/manual evidence quality, and Markdown/JSON consistency.
...
- `1.71.2` tightens Adoption Execution Assurance evidence: simulation steps must include exit code, read-only marker, target write marker, target diff status, output digest, and typed source-status inputs before full adoption can be claimed.
- `1.71.1` hardens Adoption Execution Assurance: old-project adoption claims must include source-system evidence, consistent Markdown/JSON state and surfaces, and a concrete read-only simulation step trace before Codex can say IntentOS is fully adopted.
...
- IntentOS fixture checks use a typed matrix covering golden, bad, migration, CLI, init/update, and output-quality cases.
