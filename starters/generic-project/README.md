...
Tell Codex the product goal in plain language. The controlled initializer has
already preserved that goal as the only current Work Queue item and verified
that this project can run IntentOS from its own installed assets.

Codex now derives onboarding depth, stack, platform, baseline, risk controls,
review depth, verification, and the first useful slice. The user is asked only
for a business fact that cannot be inferred, an unavailable external fact, a
genuine product preference, or consent to one prepared real-world effect.

Do not ask the user to choose an internal command, starter, baseline, pack,
