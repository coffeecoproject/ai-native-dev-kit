# iOS Project Agent Working Agreement

## Mission

This repository follows an AI-native, spec-first development workflow for iOS app development.
...
6. Do not modify auth, permission, persistence migration, production config, secrets, entitlements, signing, Keychain, or security-sensitive logic without a risk report and explicit approval.
...
Use `scripts/new-workflow-item.mjs` to create numbered request, preflight, spec, eval, task, AI task log, review packet, GPT review prompt, review loop report, follow-up proposal, and final report files instead of hand-copying templates.
...
Update `scripts/verify.sh` after scheme, workspace/project, and simulator destination are known.
