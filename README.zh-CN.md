...
版本说明：`1.4.0` 是历史上的 Project Memory 阶段；当前主线是 `1.56.x`，重点是统一任务收口判断、解释为什么得到这个判断，把它转换成上线评审视图，并在人工批准后生成受控发布执行计划。

1.56.0 加入 Release Execution Protocol：上线评审通过并且人明确批准后，Codex 可以生成发布执行计划，说明步骤、负责人、停止条件和证据要求。它不批准上线，也不会自己部署。
