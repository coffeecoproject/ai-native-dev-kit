...
版本说明：`1.4.0` 是历史上的 Project Memory 阶段；当前主线是 `1.50.x`，重点是证据引用解析，避免业务规则还有关联面没处理完，或引用了不存在的证据文件就被误认为完成。

1.50.0 强化收口证据：Codex 可以要求 `DONE` 证据引用真实可解析，可以从 git diff 读取变更文件，也可以要求执行收口必须引用严格的变更影响覆盖报告。
