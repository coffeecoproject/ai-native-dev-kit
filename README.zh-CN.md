...
它不是让 Codex 自动继续做更多事，而是让 Codex 先判断当前目标应该走哪条路：
...
Goal Card 只负责路线选择，不代表批准实现、批准风险、批准发布，也不会绕过 request、preflight、spec、eval、task、Engineering Baseline、Review Loop 或 Human Approval。

## Subagent Orchestration

从 `0.32.0` 开始，新增 Subagent Orchestration。

它不是要求每次都开多个 agent，而是当 Codex 使用 helper agent 时，必须说清楚：
...
17. 如果使用 helper agent，生成 `subagent-run-plans/`，关闭或跳过所有 subagent 后运行 `node scripts/check-subagent-orchestration.mjs .`。
18. 如果 Codex 提出后续建议，运行 `node scripts/check-next-step-boundary.mjs . --task <task-card>`，确认建议没有越界。
19. 合并后写 AI task log。
20. 阶段性复盘进入 workflow retros。
21. 重复问题进入 workflow improvements。
22. 重复执行模式进入 skill candidates，但不能自动启用 active Skill。
...
BL2 不等于“已经生产就绪”。工业包有五档成熟度：`draft`、`candidate`、`stable`、`deprecated`、`retired`。当前内置 pack 都保持 `draft`，需要真实项目 dogfood、误报记录、严格检查和维护者复查后才可能升级。
...
每个工业包都带有 `packVersion`、`minimumDevKitVersion`、`lastReviewedAt`、`stabilityNotes` 和 `maturity`，用于区分 draft 成熟度、证据状态和后续兼容要求。包内还包含 `maturity.md`、`evidence.md`、`dogfood.md`、`false-positive-log.md`、`owner.md` 和 `changelog.md`。
