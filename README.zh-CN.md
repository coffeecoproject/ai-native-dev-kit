...
版本说明：`1.4.0` 是历史上的 Project Memory 阶段；当前主线是 `1.55.x`，重点是统一任务收口判断、解释为什么得到这个判断，并把它转换成上线评审视图。

1.55.0 加入 Launch Review View：用户问“能不能上线/提交审核”时，Codex 可以基于收口结果整理上线评审视图，说明还缺什么、谁来确认、下一步怎么做。它不批准上线，也不部署。
