...
版本说明：`1.4.0` 是历史上的 Project Memory 阶段；当前主线是 `1.58.x`，重点是统一任务收口判断、上线评审、小白可用的发布引导，以及人工批准后的受控发布执行计划。

1.58.0 加入 Release Guide Consolidation：用户说“帮我上线”时，Codex 会用一个入口自动路由到发布适配、上线评审、结构化审批和发布执行计划，不需要用户理解内部命令。它不批准上线，不自己部署，不运行云平台发布命令，也不让用户贴密钥。
