...
它不是提示词合集，也不是代码模板、框架脚手架或部署工具。当前正式支持的平台是 Codex；Claude 和 Cursor 文件只作为兼容参考，不代表具备当前 IntentOS 的完整行为一致性。

> 你只说真实业务；AI 做全部技术判断并完成内部流程。只有缺少业务事实或即将产生现实影响时，才需要你回应。

## 30 秒开始

前置要求：Node.js `22.x`（正式支持范围为 `>=22 <23`）。
...
1.113.0 收口 1.112 审查发现的 P0/P1 消费链问题。必需检查现在使用明确的
类型化结果，每个任务都有与风险相称但不会消失的验证和完成链，Completion
Evidence 会重新验证当前执行证据，公开 finish 只保留一个失败即阻断的技术权威。
新项目、生成项目、老项目下一次会话、受控写入中断恢复和严格发布准备都有行为
证据，同时没有增加公开流程，也不让用户承担技术判断。
老项目接入会把原始自然语言目标、项目身份、当前版本、源码状态、规范计划和精确
可逆动作图绑定成短时、一次性的本地权限。它保留旧 Agent 规则并建立 Codex 标准
入口，但不能授权业务代码、CI、Hook、发布、生产、密钥、付费资源或不可逆数据操作。

1.112.0 对 IntentOS 的九个能力领域和十九条必需消费链进行了完整审查。
审查确认现有体系已经具备大量严格能力，同时发现 1 个可能错误报告任务完成的
P0，以及 5 个“严格证据没有被公开消费者完整执行”的 P1 根因。这一版只发布
可复现的审查证据，不声称行为闭环已经完成；必须先由 1.113 收口这些问题，之后
才进入仓库结构治理。

1.111.1 收口 Codex 实际读取、生成和安装内容里的责任漂移。技术路线、方案、
基线、风险处理、复查、验证、受控写入和发布就绪建议都由 Codex 自动完成；用户
只补真实业务事实、有效产品偏好、某个已准备现实动作的明确同意，或项目无法获得
的外部事实。旧字段仍可兼容读取，但没有新增六阶段流程或公开命令。
...
被当成可靠证明。只有当前任务或接入结论确实依赖它时，Codex 才会自动核对
具体规则、覆盖范围、真实拦截能力、当前代码身份和安全边界。无关任务保持轻量，
项目中更强且已证明有效的规则继续保留，用户不需要选择检查器或验证方式。

1.109.0 收口项目入口和接入可信链。用户只给一个自然语言目标，所有公开入口
共同使用同一份目标路径、项目身份、项目事实、当前指导和进行中任务判断。
空目录或未创建目录通过可回滚的原子事务完成初始化，并生成项目本地验收回执；
老项目通过同一轮严格证据完成接入判断，不再让用户选择迁移深度或技术治理方式。
已有改动只阻断真正冲突的写入；新生成项目必须能独立启动 IntentOS 并路由第一个
普通任务，之后才能声称工作方式已激活。

1.108.0 增加按风险触发的“业务全集覆盖”。当项目证据表明相关类别、来源、
处理路径、生命周期分支、选择点或派生一致性关系存在遗漏风险时，Codex 必须
追踪所有相关场景，并为排除项提供项目证据，之后才能进入业务闭环、影响面、
测试计划和最终收口。该过程由 `work` 内部自动编排，用户不需要选择检查器，
也不负责判断技术完整性。

1.107.1 收口实际生效指导的分发漂移：检查范围从手工登记文件扩展到 Codex
真实读取、引用和生成的内容。Codex 是唯一正式支持的 Agent 平台；Claude 和
Cursor 仅保留兼容参考。老项目迁移报告也不再把技术治理选择交给用户。

1.107.0 补齐发布拓扑迁移治理：项目文件调整、非生产演练、切换、发布、
切换后证明和旧通道退役必须分别验证，任何报告都不能自行授权生产操作。

1.106.0 让严格发布链绑定同一份当前拓扑、源码、候选版本、软件包和具体动作，
复制、过期、缺失或冲突的证据不能解锁发布准备状态。

1.105.0 增加平台无关的发布拓扑视图：把源码、编排、执行环境、软件包传输、
证据存储和真实目标分开识别，再由 IntentOS 给出一个技术建议。用户不需要选择
Runner、协议或平台组件；这份视图也不会授权发布或生产操作。
...
浅层扫描漏掉治理文件，基线必须有真实证据，安装项目的 CI 也会按实际改动执行
当前任务与发布检查。
