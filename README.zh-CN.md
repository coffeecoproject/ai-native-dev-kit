...
版本说明：`1.4.0` 是历史上的 Project Memory 阶段；当前主线是 `1.51.x`，重点是收口证据精确性，避免拿旧报告、弱证据或未解析的记录引用来证明当前任务已经完成。

1.51.0 强化严格收口：Codex 可以要求执行收口链接到正确的变更影响覆盖报告，确认它匹配当前任务或意图，并拒绝弱证据文件或无法解析的 `artifact:` / `human-decision:` 记录。
