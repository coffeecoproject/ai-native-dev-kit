...
版本说明：`1.4.0` 是历史上的 Project Memory 阶段；当前主线是 `1.52.x`，重点是引导式任务收口，让用户不用选择内部证据命令，也能判断任务是否能算完成。

1.52.0 加入 Guided Closure Experience：Codex 可以用一张只读卡片说明任务是否能收口、还缺什么、下一步能做什么、哪些需要人判断，以及技术证据在哪里；底层严格证据检查继续保留给维护者和 CI 使用。
