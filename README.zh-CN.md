...
发布记录：[releases/1.99.0/release-record.md](releases/1.99.0/release-record.md)。
...
下方历史版本说明会保留早期的“负责人”“人工批准”等机器兼容词汇，作为审计历史。
在 1.99 的公开交互里，这些词不代表个人开发者需要组建团队，而会被翻译成：缺少的
业务/外部事实、当前用户对某个具体现实影响的同意，或仅供系统内部使用的责任领域。
...
`work` 会明确阻断；严格检查也不能再被空报告参数或替换后的本地 schema 绕过。

受控配置计划只能留在目标项目内，并绑定完整当前状态；执行失败会回滚已尝试写入，
Apply Receipt 还会重新运行安装入口和基线检查。发布批准不能替代 Launch Review，
发布包身份使用统一词汇，基线包必须匹配平台，生成项目也会真实运行公开的
release-channel 命令。这些能力仍不批准代码实现、apply、发布或生产操作。
...
以及什么变化会让结论失效；普通用户看到的白话下一步也由同一次动作选择生成。

这个决策仍是派生视图，不是调度器、证据权威、审批、任务状态变化或执行许可，
也不批准实现、apply、提交、推送、发布或生产操作。

1.95.0 把项目入口和日常任务收敛成一个只读 Operating Model。用户只说目标，
IntentOS 自动判断新项目、老项目接入、继续任务、查看进度、任务收口或发布准备，
然后只给一个当前状态、一个安全下一步，以及最多一个真正需要用户决定的问题。
底层任务、证据、审批、基线、接入和发布系统继续保留权威，并通过 Evidence Trace
和 Authority Recommendation 说明判断来源与负责人建议。

这个入口不会新增一套平行证据，不会写项目文件、改变任务状态、批准实现或批准发布。
项目使用 BL2 也不代表每个低影响任务都要自动升级成高影响流程。

1.94.0 收口项目基线和发布清单。Codex 会自己判断技术平台、BL 档位和具体基线包，
把选择写进精确受控的初始化计划，并在执行后检查选中的基线是否真的安装到位。
老项目会保留更严格且有效的原规则，只对缺口生成受控补齐计划，不做整套覆盖。

Manifest 现在会按真实严格 schema 校验，重复复制规则已清理，`.intentos` 管理文件
不会再误导项目识别，旧的基线直接 apply 入口也不会再写文件。草案基线包仍明确
标为草案，安装完成不等于产品正确或可以直接上线。

1.93.0 收口发布授权可信链。现在只有结构化的人工 Release Approval Record，
并且它与当前项目、Git 版本、候选包、发布目标、包身份和严格发布证据完全一致时，
才能进入受限发布协助。聊天里写“同意发布”、命令行自报 `APPROVED`、Git tag，
或复制旧项目的批准记录，都不能解锁发布执行。

这仍不代表 IntentOS 批准或保证生产发布。部署、应用商店/小程序提交、迁移、
密钥、DNS、支付、权限、生产配置和回滚执行，继续由发布负责人或项目外部发布系统负责。
...
失败，不再被当成可跳过。普通非严格项目检查仍保持轻量；目标项目位于大仓库子目录
时，外层无关脏文件也不会再污染该项目的任务风险判断。

1.91.1 仍然不授权执行。它不批准代码实现、apply、提交、推送、发布、生产、
...
目标路径绑定规则。Adoption apply-chain 也会在共享 helper 层重新计算 apply
plan digest，避免 resolver 先把伪造 digest 的链路说成 verified。

1.89.2 仍然不授权执行。它会拒绝弱批准、过期批准、伪造 digest 或机器批准的
apply 证据，但不批准代码实现、native apply、提交、推送、发布、生产、测试、
迁移、外部平台动作或项目负责人决策。
