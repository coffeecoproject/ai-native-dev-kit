...
版本说明：`1.4.0` 是历史上的 Project Memory 阶段；当前主线是 `1.48.x`，重点是变更影响覆盖，避免业务规则只改到某一层就被误认为完成。

1.48.0 增加 Change Impact Coverage：当任务涉及校验、输入限制、API、后端规则、数据、权限或用户提示时，Codex 要记录哪些相关面必须处理、哪些可以排除、哪些需要人判断。
