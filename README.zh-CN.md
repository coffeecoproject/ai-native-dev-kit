...
版本说明：`1.4.0` 是历史上的 Project Memory 阶段；当前主线是 `1.57.x`，重点是统一任务收口判断、上线评审、小白可用的发布适配，以及人工批准后的受控发布执行计划。

1.57.0 加入 Guided Release Adapter：用户想上线但不懂发布流程时，Codex 可以自动发现项目发布方式，推荐安全的第一步，说明还缺什么，并接到发布执行计划。它不批准上线，不自己部署，也不让用户贴密钥。
