...
版本说明：`1.4.0` 是历史上的 Project Memory 阶段；当前主线是 `1.59.x`，重点是统一任务收口判断、上线评审、小白可用的发布引导、平台发布配方，以及人工批准后的受控发布执行计划。

1.59.0 加入 Platform Release Recipes：用户说“帮我上线”时，Release Guide 可以根据平台映射到 Web 预览、后端/API 交接、小程序审核交接等发布准备清单。配方只是只读地图，不批准上线，不部署生产，不运行云平台命令，不上传包，也不让用户贴密钥。
