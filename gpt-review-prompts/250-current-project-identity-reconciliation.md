# GPT Review Prompt: 250-current-project-identity-reconciliation
...
Verification Gaps:
- None if all required evidence is present; otherwise name the exact missing check.
...
NO_ACTION Reasons:
- State a concrete reason for every NO_ACTION finding.

Reviewer Notes:
- None if no additional note applies.
```
