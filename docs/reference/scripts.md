...
`scripts/resolve-business-universe-coverage.mjs` is the 1.108 conditional internal coverage resolver. It consumes exact Task Governance routing and project evidence, derives task-relevant categories, participants, origins, lifecycle applicability, project-native paths, selection and consistency relationships, stable coverage scenarios, and required proof strength. Deep or interrupted scans retain an inventory digest, cursor, candidate set, and canonical resume-state digest; maintainers may continue the same trusted scan with `--resume-from <artifact-ref>`, `--max-files`, and `--max-bytes`, while users remain on the natural-language `work` entry. A task routed `NOT_REQUIRED_WITH_REASON` does not create an empty report. `scripts/check-business-universe-coverage.mjs` validates exact Work Queue and Task Governance identity, lifecycle/provenance completeness, scenario identity, discovery-resume integrity, Challenger result, Evidence Authority, digest, and no-authority boundaries. Ordinary users do not choose or call this layer.
