...
| `resolve-verification-runtime-plan.mjs` | Read-only runtime-trust planner that derives the required trust level and a project-evidenced adapter contract, preflight probes, isolation, and ownership controls without asking the user to choose infrastructure |
| `check-verification-runtime-plan.mjs` | Verification Runtime Plan schema, task/source identity, tier, adapter discovery, contract digest, capability, control, preflight, isolation, and no-authority checks |
| `check-verification-run-manifest.mjs` | Verification Run Manifest source revision, service instance, isolated resource, run ownership, test output, cleanup, Evidence Authority, and no-overclaim checks |
...
| `check-release-approval-record.mjs` | Project/Git/candidate/package-bound human release approval and strict upstream authority-chain validation |
| `resolve-release-execution.mjs` | Read-only release execution planner derived from the exact structured release trust chain |
| `check-release-execution.mjs` | Release Execution Plan authority binding, step ownership, evidence, and no-auto-production checks |
...
- `check-verification-runtime-plan.mjs` validates the Codex-selected runtime trust level and current project-evidenced adapter before a run. LOW binds source and output; MEDIUM adds adapter-specific service identity; HIGH requires isolated runtime controls; POSSIBLE_HIGH remains blocked until classified. It never starts services or grants execution authority.
- `check-verification-run-manifest.mjs` validates that recorded verification came from the current project revision and the exact Runtime Plan, with environment preflight, platform-appropriate service identity, run-owned resources, isolated data/session controls where required, positive/negative paths, output digests, and bounded cleanup. It does not run tests and does not prove product correctness by itself.
...
- `check-release-approval-record.mjs` allows an advisory empty-project skip only in default mode. Strict mode fails closed and rejects copied, stale, expired, non-human, wrong-project, wrong-revision, wrong-candidate, wrong-target, wrong-package, unsafe-action, weak-upstream, symlinked, or project-external release authority.
- `check-release-execution.mjs` allows old plan-only Markdown compatibility, but `--require-release-trust` rejects real execution without the exact structured Release Approval Record, strict Release Evidence Gate, current Runtime Hygiene, bound Release Channel Policy, and required strict platform handoff evidence. It also rejects high-risk production actions assigned to Codex, approval claims, and attempts to treat prose, CLI status, Launch Review View, tags, or GitHub Releases as approval.
...
- `check-baseline-state.mjs` allows empty projects, but rejects no-code/new-project baselines marked `CONFIRMED` without project-owned evidence or a verifiable external fact, and rejects implementation permission that claims bounded writes without evidence.
...
`check-verification-runtime-plan.mjs` validates required trust and adapter selection. `check-verification-runtime-lifecycle.mjs` validates exact executable actions and ownership boundaries. `check-verification-run-manifest.mjs` validates only observed run output, service identity, resource ownership, and cleanup evidence. A pass at one layer cannot replace either of the other layers.

In the `1.104.0` strict completion chain, `check-test-evidence.mjs`,
`check-execution-assurance.mjs`, and `check-completion-evidence.mjs` accept
`--require-runtime-trust`. Each invokes the authoritative Run Manifest checker
