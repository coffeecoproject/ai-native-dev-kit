...
| `check-change-impact-coverage.mjs` | Change Impact Coverage report completeness, structured evidence, preflight/closure mode, cross-surface close-out, evidence, evidence reference resolution, changed-file implication, and overclaim checks |
...
- `check-execution-closure.mjs` accepts `--require-impact-coverage` when cross-surface READY closures must cite strict Change Impact Coverage evidence.
