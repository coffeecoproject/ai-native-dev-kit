...
| `resolve-completion-evidence.mjs` | Read-only final completion evidence gate that binds BRC, Verification Plan, Test Evidence, and Execution Assurance source artifacts |
| `check-completion-evidence.mjs` | Completion Evidence source-chain, source schema, source digest, source intent, task, outcome, and no-authority checks |
