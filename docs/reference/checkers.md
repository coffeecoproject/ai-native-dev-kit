...
| `check-controlled-apply-readiness.mjs` | Controlled Apply Readiness boundary, approval, high-risk, rollback, verification, and structured evidence digest checks |
| `check-approval-record.mjs` | Approval Record human-owner, action ID, target path, plan hash, expiry, rollback, verification, structured evidence, and non-authorization checks |
