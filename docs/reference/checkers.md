...
| `resolve-product-completeness.mjs` | Read-only product completeness resolver for idea, first-slice, runnable MVP, internal-trial, release-review, or blocked state, with optional smoke evidence |
...
- `check-mvp-example.mjs` checks bundled local MVP examples. It proves the example is coherent and locally testable; it does not prove any target project is production-ready.
...
- `check-apply-plan.mjs`, `check-controlled-apply-readiness.mjs`, and `check-approval-record.mjs` accept `--require-structured-evidence` for new structured artifacts that must include `Machine-Readable Evidence`.
