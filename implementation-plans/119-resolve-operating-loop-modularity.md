...
Keep `scripts/resolve-operating-loop.mjs` as the single public entry and move classification, decision, identity, presentation, state, source execution, and source orchestration into explicit modules under `scripts/operating-loop/`. Register the complete module graph in the Manifest and workflow-version inventory, and make self-check consumers inspect the full modular source graph. The final full-chain review may also update `scripts/check-test-evidence.mjs`, `scripts/check-execution-assurance.mjs`, `scripts/check-completion-evidence.mjs`, `scripts/check-runtime-hygiene.mjs`, `scripts/check-business-universe-coverage.mjs`, `scripts/lib/release-trust.mjs`, `scripts/lib/release-topology-consumer.mjs`, `tests/test-evidence-batch-authority.test.mjs`, and `tests/release-trust-boundary.test.mjs` only to preserve historical batch evidence and its internal release or semantic-locator bindings without granting it current authority, and to prove that the exact release preflight check can be replayed both before commit from a non-empty staged candidate and after commit from a clean `base_revision..HEAD` candidate. Explicit historical selection must remain current-strict, and dirty post-commit worktrees must remain fail-closed. It may update `scripts/lib/evidence-authority.mjs` and its distribution-trust regression test solely to classify Release Review Provenance as governed workflow output, preventing the required review artifact from recursively changing the source identity it attests. The same review may consolidate report authority through `scripts/lib/report-authority.mjs`, migrate `scripts/check-plan-review.mjs`, `scripts/check-business-universe-coverage.mjs`, and `scripts/lib/plan-review-binding.mjs` to that shared rule, and update the two affected `package.json` verification routes. This consolidation is limited to removing filename-order authority and preserving explicit current selection versus historical batch audit semantics. The final evidence closeout may update `schemas/artifacts/execution-assurance.schema.json`, `scripts/lib/execution-assurance-consumer.mjs`, and `tests/current-trust-fixture.test.mjs` only to admit an exact committed `git:<base revision>` candidate after source commit; mutable refs, dirty non-governance files, non-ancestor bases, and empty committed intervals remain fail-closed.

The same bounded authority correction includes `scripts/check-release-execution-topology.mjs` and `tests/release-execution-topology.test.mjs`: batch discovery audits historical reports without replaying them as current, while explicit report selection remains current-strict. The final governed evidence set retains `releases/1.119.0/independent-review-report.md` as the independent human-readable review record.

## Boundaries

The final evidence closeout may also synchronize the trusted Execution Assurance Schema digest in `scripts/lib/artifact-schema.mjs` and harden committed-candidate dirty-state detection against Git rename records. This compatibility work remains limited to `schemas/artifacts/execution-assurance.schema.json`, `scripts/lib/artifact-schema.mjs`, `scripts/lib/execution-assurance-consumer.mjs`, and `tests/current-trust-fixture.test.mjs`; governed evidence rotation is permitted, while mutable refs, dirty non-governance files, non-governance renames, non-ancestor bases, and empty committed intervals remain fail-closed. The runtime retry closeout may update `tests/execution-distribution-trust.test.mjs` solely so each test reclaims its registered, direct-child `intentos-*` temporary roots and exposes cleanup failure. It must not add a broad temporary-directory cleaner or weaken test assertions.
...
6. Preserve the already-published behavior characterization while correcting only stale or demonstrably pre-existing test-oracle defects, including the Test Evidence, Execution Assurance, Completion Evidence, and Runtime Hygiene batch/current authority distinctions found by the final full-chain review. Cover Runtime Hygiene release-preflight replay before and after commit in `tests/release-trust-boundary.test.mjs`, without weakening dirty-worktree rejection.
...
9. Apply the same historical-batch/current-selection authority split to Release Execution Topology and cover it with focused positive and negative tests.
10. Keep the trusted Execution Assurance Schema digest synchronized and prove committed-candidate replay across governed and non-governed rename states.
11. Make the distribution-trust fixture reclaim its own registered temporary roots after every test, with path ownership checks and fail-visible cleanup errors.
12. Generate bounded governance evidence, retain the independent review report, and preserve one final trusted runtime; retain no standalone full-verification aggregate log.
...
- `node --test tests/release-trust-boundary.test.mjs`
- `node --test tests/current-trust-fixture.test.mjs`
- `node --test tests/execution-distribution-trust.test.mjs`
- `npm run verify:release-topology`
