...
`resolve-execution-closure.mjs` is read-only. It reports changed scope, verification closure, review surface closure, debt closure, and commit readiness. `check-execution-closure.mjs` rejects closure overclaims, missing required surfaces, and commit-readiness without passing verification evidence.
