...
export const SOURCE_CONFIG = Object.freeze({
  ...
  CHANGE_IMPACT: config("change-impact-coverage-reports", "change-impact-coverage.schema.json", "scripts/check-change-impact-coverage.mjs", ["--require-structured-evidence", "--resolve-evidence-refs"], "impact_digest", "outcome", ["CHANGE_IMPACT_RECORDED"]),
...
export function findSourceReport(projectRoot, sourceKind, explicitRef, taskRef, intentDigest) {
  ...
  for (const root of roots) {
    ...
    for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
      ...
      const taskMatch = !taskRef || !evidence.task_ref || evidence.task_ref === taskRef;
      const intentMatch = !intentDigest || !evidence.intent_digest || evidence.intent_digest === intentDigest;
      candidates.push({ file, taskMatch, intentMatch, mtime: fs.statSync(file).mtimeMs });
    }
  }
  candidates.sort((a, b) => Number(b.taskMatch && b.intentMatch) - Number(a.taskMatch && a.intentMatch) || b.mtime - a.mtime);
...
export function validatePlanningSource({ projectRoot, sourceKind, file, taskRef, intentDigest, kitRoot }) {
  ...
  const valid = checker.ok && taskMatch !== "No" && intentMatch !== "No";
  return {
    ...
    reason: !checker.ok ? checker.reason : taskMatch === "No" ? "Report task does not match the current task." : intentMatch === "No" ? "Report intent does not match the current intent." : ready ? "The source passed its strict checker and is ready." : `The source is valid but remains ${state}.`,
    evidence,
  };
}
...
export function buildExecutionEntryContract({ authorityBinding, taskRef, intentDigest, taskImpact, reportRef, closureDigest, sources }) {
...
export function validatePlanningClosureEvidence(projectRoot, reportFile, evidence) {
  const errors = [];
  ...
  for (const source of evidence.source_requirements.filter((item) => item.required === "Yes")) {
    ...
    if (source.current_task_match !== "Yes" && !tasklessSource(source.source_kind)) errors.push(`${source.source_kind} does not match the current task`);
    if (source.current_intent_match === "No") errors.push(`${source.source_kind} does not match the current intent`);
  }
...
function tasklessSource(kind) {
  return kind === "CHANGE_IMPACT";
}

function taskMatchFromNested(evidence, taskRef) {
  const refs = [evidence.task_governance?.task_ref, evidence.authority_binding?.task?.task_ref, evidence.business_rule_ref && evidence.task_ref].filter(Boolean);
...
function intentMatchFromNested(evidence, intentDigest) {
  const values = [evidence.authority_binding?.task?.intent_digest].filter(Boolean);
