...
const unknown = unknownOptions(args, new Set(["json", "intent"]));
...
const report = buildReport(projectRoot, intent);
...
function buildReport(root, userIntent) {
  ...
  const state = hasCode && hasRun && hasTest ? "RUNNABLE_MVP" : hasFirstSlice ? "FIRST_SLICE_DEFINED" : "IDEA_ONLY";
  return {
    ...
    checklist: [
      ...
      ["Verification evidence", hasTest ? "pass" : "fail", hasTest ? "package script" : "not detected"],
...
function printReport(report) {
  ...
  console.log(`| Core flow verification | ${report.state === "RUNNABLE_MVP" ? "pass" : "not verified"} | test or smoke command |`);
  console.log("");
  console.log("## Gaps");
  console.log("");
