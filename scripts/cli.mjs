...
const commandRegistry = {
  ...
  "verification-runtime-plan": {
    description: "Derive the runtime identity, isolation, preflight, ownership, and cleanup proof required by the current task tier.",
    script: "scripts/resolve-verification-runtime-plan.mjs",
    writes: false,
    buildArgs: (args) => withDefaultTarget(args),
  },
  "verification-runtime-plan-check": {
    description: "Check Verification Runtime Plans for current task, source, tier, adapter, and fail-closed control binding.",
    script: "scripts/check-verification-runtime-plan.mjs",
    writes: false,
    buildArgs: (args) => withDefaultTarget(args),
  },
  "verification-runtime-lifecycle": {
    description: "Derive the exact bounded local execution and cleanup plan for a ready Verification Runtime Plan.",
    script: "scripts/resolve-verification-runtime-lifecycle.mjs",
    writes: true,
    buildArgs: (args) => withDefaultTarget(args),
  },
  "verification-runtime-lifecycle-check": {
    description: "Check lifecycle actions, project binding, resource ownership, environment, and cleanup boundaries.",
    script: "scripts/check-verification-runtime-lifecycle.mjs",
    writes: false,
    buildArgs: (args) => withDefaultTarget(args),
  },
  "verification-runtime-run": {
    description: "Execute one bounded local lifecycle plan and emit observed run and cleanup evidence.",
    script: "scripts/run-verification-runtime.mjs",
    writes: true,
    buildArgs: (args) => withDefaultTarget(args),
  },
  "verification-run-check": {
    description: "Check Verification Run Manifests for observed runtime identity, isolation, ownership, execution, and cleanup evidence.",
    script: "scripts/check-verification-run-manifest.mjs",
    writes: false,
    buildArgs: (args) => withDefaultTarget(args),
  },
  "test-evidence": {
    description: "Bind exact current-run outputs to a Verification Plan without approving completion.",
    script: "scripts/resolve-test-evidence.mjs",
    writes: false,
    buildArgs: (args) => withDefaultTarget(args),
  },
  "test-evidence-check": {
    description: "Check Test Evidence coverage, freshness, Runtime Trust binding, and no-overclaim boundaries.",
    script: "scripts/check-test-evidence.mjs",
    writes: false,
    buildArgs: (args) => withDefaultTarget(args),
  },
  "completion-evidence": {
    description: "Gate final completion against business, verification, execution, and one current Runtime Trust run.",
