...
| --- | --- | --- |
| Production config | `infra/production.env` | preserve |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `R-001` | `infra/production.env` | line 1: Production endpoint is owned by operations. | `PRODUCTION_CONTROL` | ops owner | preserve and escalate | preserve | Production config is outside IntentOS ownership. | production | preserve | Yes |

## Conflicts And Decisions

| Conflict ID | Conflict Type | Existing Source | IntentOS Target | Default Decision | Human Decision Required |
| --- | --- | --- | --- | --- | --- |
| `C-001` | `PRODUCTION_CONFLICT` | `infra/production.env` | IntentOS migration | Preserve production config | Yes |

## Proposed Native Migration Plan

| Step | Action | Exact Target Path | Writes Target Files? | Requires Human Approval | Status |
| --- | --- | --- | --- | --- | --- |
| 1 | changes production config during migration | `infra/production.env` | No | Yes | Proposed |
...
| Item | Action | Reason |
| --- | --- | --- |
| Production config | Change | Bad fixture |

## Restore Plan

| Field | Value |
| --- | --- |
| Backup path | `.ai-native/backups/native-migration/bad/production.env` |
| Restore method | Restore backup |
| Restore owner | ops owner |
...
- This plan changes production config, secrets, migrations, payment, permissions, data, provider state, legal, tax, finance, HR, security, privacy, or compliance behavior: Yes
