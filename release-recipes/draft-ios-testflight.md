# iOS TestFlight Release Recipe

## Human Summary

| Field | Value |
|---|---|
| Recipe ID | `ios-testflight-handoff` |
| Recipe Status | `DRAFT` |
| Platform Family | `ios` |
...
| Signal | Evidence |
|---|---|
| iOS app | `.xcodeproj`, `.xcworkspace`, `Package.swift`, or iOS docs |
...
| `app-store` | `HUMAN_OR_EXTERNAL_SYSTEM` |
...
| Decision | Owner |
|---|---|
| app-store upload and submission | `HUMAN_REQUIRED` |

## Codex Allowed Actions

| Action | Risk Class |
|---|---|
| inspect local iOS project | `LOCAL_READ_ONLY` |
...
```bash
node scripts/cli.mjs release-execution . --intent "prepare iOS release handoff" --mode PLAN_ONLY
```
