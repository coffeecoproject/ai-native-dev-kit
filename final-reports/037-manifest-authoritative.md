...
Phase `0.37.0` makes the manifest the source of truth for intentos asset lists and safe static copy rules. The important boundary is that manifest authority does not approve risky mutations: PR template governance, AGENTS governance, industrial pack selection, migration, package publishing, and init/update plan behavior stay outside this phase.

## Completed

- Changed `intentos-manifest.json` to authoritative mode.
...
- Updated `check-intentos.mjs` to read source required files from manifest.
...
Evidence refs: `intentos-manifest.json`, `scripts/check-manifest.mjs`, `scripts/check-intentos.mjs`, `scripts/init-project.mjs`, and `review-loop-reports/037-manifest-authoritative.md`.
...
The manifest loader resolves root `intentos-manifest.json` in the source repository and `.intentos/intentos-manifest.json` in generated projects. The generated-project scripts use the copied manifest and loader, so required target assets can be added through manifest without editing the check and next-step scripts.
